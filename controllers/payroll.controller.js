...
export const bulkGeneratePayslips = async (req, res) => {
  try {
    ...
    const allEmployees = await Employee.find({}); // Fetches ALL employees
    ...
    for (const employee of allEmployees) {
      const attendanceRecords = await Attendance.find({ employee: employee._id, date: { $gte: startDate, $lte: endDate } }).populate('onLeaveRequest', 'leaveCategory');
      const grossSalary = employee.salary;
      const totalDaysInMonth = new Date(year, month, 0).getDate();
      const perDaySalary = grossSalary / totalDaysInMonth;
      let nonPayableDays = 0;
      ...
      attendanceRecords.forEach(record => {
        if (record.status === 'Absent') { nonPayableDays += 1; deductionDetails.push({ date: record.date, reason: 'Absent' }); }
        else if (record.status === 'Half Day') { nonPayableDays += 0.5; deductionDetails.push({ date: record.date, reason: 'Half Day' }); }
        else if (record.status === 'On Leave' && record.onLeaveRequest?.leaveCategory === 'Unpaid') { nonPayableDays += 1; deductionDetails.push({ date: record.date, reason: 'Unpaid Leave' }); }
      });
    ...
    res.status(200).json({ message: `Bulk generation complete.`, payslips: generatedPayslips });
